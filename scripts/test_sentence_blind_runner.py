...
def main() -> int:
    samples = [
        ...
        {
            "id": "C-03", "mode": "prose", "context": "general",
    ...
    if candidate_ids != {"A-01"} or cleared_ids != {"B-02"} or checks_not_run_ids != {"C-03"}:
        print(json.dumps(result, indent=2), file=sys.stderr)
        raise AssertionError("runner did not separate UI recovery, guarded safety copy, and the unrun manual prose check")
    ...
    print("PASS: deterministic blind runner separates UI defects, guarded copy, and required manual prose checks")
    return 0
